...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 2. إرسال نوتيفيكيشن جديد
    elseif ($action == "send_notif") {
        $msg = $_POST['msg'];
        if (!empty($msg)) {
            $stmt = $conn->prepare("INSERT INTO notifications (msg, year, created_at) VALUES (?, 'first', ?)");
    ...
    // 4. حذف المواد (Resources)
    elseif ($action == "delete_resource") {
        $id = $_POST['id'];
        // حذف الملف من السيرفر الأول
    ...
    // التوجيه لصفحة الأدمن بتاعت السنة الأولى
    header("Location: FIRST_iyuroghiuwhertigwbertiogbsdfkhjvdsrtiohbvskthjbvisuprbtgskghbskdjfghtgstrougi.php");
    exit();
}