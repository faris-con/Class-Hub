...
// 3. جلب بيانات مادة Graphics فقط
$sql = "SELECT * FROM resources WHERE subject_id = 'graphics' ORDER BY id DESC";
...
<head>
    ...
    <title>Computer Graphics - Class Resources Hub</title>
    ...
    <link rel="stylesheet" href="graphics-styles.css?v=<?php echo time(); ?>">
...
<body>
    ...
    <section class="hero">
        <h1 class="hero-title">🎨 Computer Graphics</h1>
        <p class="hero-subtitle">Master visual computing and rendering techniques</p>
                        ...
                        <?php foreach($summaries as $sum): ?>
                            <div class="resource-row">
                                <a href="../<?php echo $sum['link']; ?>" target="_blank" class="resource-link">
                                    <span class="link-icon-small">📑</span>
                                    <?php echo htmlspecialchars($sum['title']); ?>
                                </a>
                                <button class="finish-btn" 
    ...
    <script>
        ...
        if(lecturesDropdown) {
            lecturesDropdown.querySelector('.dropdown-toggle').addEventListener('click', (e) => {
                e.preventDefault(); lecturesDropdown.classList.toggle('active');
            });
        }
        document.addEventListener('click', (e) => {
            if (lecturesDropdown && !lecturesDropdown.contains(e.target)) lecturesDropdown.classList.remove('active');
        });
