...
$videos = [];
$assignments = [];
// مصفوفة لتخزين كل IDs المادة دي عشان العداد الشامل
$all_subject_ids = [];

// 3. جلب بيانات مادة Network فقط
$sql = "SELECT * FROM resources WHERE subject_id = 'network' ORDER BY id DESC";
$result = $conn->query($sql);

if ($result && $result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        // تخزين الـ ID في القائمة الشاملة
...
<head>
    ...
    <title>Computer Network - Class Resources Hub</title>
    ...
    <link rel="stylesheet" href="network-styles.css?v=<?php echo time(); ?>">
...
<body>
    ...
    <section class="hero">
        <h1 class="hero-title">🌐 Computer Network</h1>
        <p class="hero-subtitle">Master network protocols, architectures and data communication</p>
                        ...
                        <?php foreach($lectures as $lec): ?>
                            <div class="resource-row">
                                <a href="../<?php echo $lec['link']; ?>" target="_blank" class="resource-link">
                                    <span class="link-icon-small">📄</span><?php echo htmlspecialchars($lec['title']); ?>
                                </a>
                        ...
                        <?php foreach($summaries as $sum): ?>
                            <div class="resource-row">
                                <a href="../<?php echo $sum['link']; ?>" target="_blank" class="resource-link">
                                    <span class="link-icon-small">📑</span><?php echo htmlspecialchars($sum['title']); ?>
                                </a>
                                <button class="finish-btn" 
                        ...
                        <?php foreach($videos as $vid): ?>
                            ...
                            <div class="resource-row">
                                <a href="<?php echo $link; ?>" target="_blank" class="resource-link">
                                    <span class="link-icon-small">▶️</span><?php echo htmlspecialchars($vid['title']); ?>
                                </a>
                                <button class="finish-btn" 
                ...
                <div class="section-content" style="display: block; width: 100%;">
                    <?php if (count($assignments) > 0): ?>
                        <?php foreach($assignments as $assign): ?>
                            ...
                            <div class="resource-row">
                                <a href="<?php echo $link; ?>" target="_blank" class="resource-link">
                                    <span class="link-icon-small">📋</span><?php echo htmlspecialchars($assign['title']); ?>
                                </a>
                                <button class="finish-btn" 
    ...
    <script>
        ...
        function updateSubjectProgress() {
            ...
            if(counterElement) counterElement.innerText = subjectFinishedCount;
        }
